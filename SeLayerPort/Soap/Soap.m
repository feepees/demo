...
+ (NSString*) createEnvelope: (NSString*) method forNamespace: (NSString*) ns forParameters: (NSString*) params withHeaders: (NSDictionary*) headers
{
	...
	[s appendFormat: @"<soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:HZduban=\"%@\">", ns];
	...
	//[s appendFormat: @"<HZduban:%@>%@</HZduban:%@>", method,[params stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"], method];
    [s appendFormat: @"<HZduban:%@>%@</HZduban:%@>", method, params, method];
	[s appendString: @"</soap:Body>"];
...
+(id)deserializeAsDictionary:(CXMLNode*)element {
	...
	for(CXMLNode* child in [element children]) 
    {
        ...
        ////// chg by niurg 2012.08.09
        //[d setObject:v forKey:[child name]];
        id reVal = [d objectForKey:[child name]];
        if (reVal != nil) {
            NSString *nodName = [NSString stringWithFormat:@"NodeName%ld", lNameNum++];
            [d setObject:v forKey:nodName];
        }
        else {
            [d setObject:v forKey:[child name]];
        }
