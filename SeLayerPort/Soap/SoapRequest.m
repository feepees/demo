...
@synthesize userInfo;       // add by niurg 2012-09-25
...
//		[self.handler performSelector: onerror withObject: error];
        [self.handler performSelector: self.action withObject:error withObject:userInfo];
	} else {
...
//        NSString *pngImageName = [NSString stringWithFormat:@"test.log"];
...
//        NSString *pngFileFullPath = [NSString stringWithFormat:@"%@/%@", pngDir, pngImageName];
//        NSError *err;
//        [response writeToFile:pngFileFullPath atomically:YES encoding:NSUTF8StringEncoding error:&err];
...
//				[self.handler performSelector: self.action withObject: fault];
                [self.handler performSelector: self.action withObject:fault withObject:userInfo];
			} else {
		...
		if(self.handler != nil && [self.handler respondsToSelector: self.action]) 
        {
            [self.handler performSelector: self.action withObject: output withObject:userInfo];
